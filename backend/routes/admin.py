...
async def reject_verification(
    ...
    # Notify user about rejection reason and how to retake verification
    try:
        ...
        if user:
            ...
            retake_url = f"{frontend_url}/verify-account"
            ...
            notification = Notification(
                ...
                subject="ID Verification Rejected - Action Required",
                content=content,
                status=NotificationStatus.PENDING,
                metadata={
                    "verification_id": verification_id,
                    "rejection_reason": reason,
                    "action_required": "retake_id_verification",
        ...
        logger.warning(f"Failed to create ID rejection notification for verification {verification_id}: {e}")
...
@router.get("/tradespeople-verifications/rejected")
async def get_rejected_tradespeople_verifications(skip: int = 0, limit: int = 20, admin: dict = Depends(require_permission(AdminPermission.VERIFY_USERS))):
    """List rejected tradespeople references verifications with rejection metadata."""
...
@router.put("/trade-questions/bulk/activate")
