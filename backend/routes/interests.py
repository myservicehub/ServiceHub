...
from ..utils.pricing import vat_inclusive_breakdown, to_decimal, coins_to_naira, to_float
...
async def pay_for_access(
    ...
    try:
        ...
        if raw_access_fee_naira is None:
            raw_access_fee_naira = coins_to_naira(to_decimal(job.get("access_fee_coins"), "15"))
        fee_breakdown = vat_inclusive_breakdown(raw_access_fee_naira)
        base_fee_naira = fee_breakdown["base_naira"]
        vat_amount_naira = fee_breakdown["vat_naira"]
        total_fee_naira = fee_breakdown["total_naira"]
        total_fee_coins = fee_breakdown["total_coins"]
        
        # Check wallet balance and deduct fee
        success = await database.deduct_access_fee(
            user_id=current_user.id,
            job_id=job["id"],
            access_fee_coins=to_float(total_fee_coins),
            access_fee_naira=to_float(total_fee_naira),
        )
        ...
        if not success:
            ...
            shortfall = to_float(total_fee_coins) - to_float(current_balance)
            
            raise HTTPException(
                status_code=400, 
                detail=f"Insufficient wallet balance. Required: {to_float(total_fee_coins):,.4f} coins (₦{to_float(total_fee_naira):,.2f}, VAT ₦{to_float(vat_amount_naira):,.2f}), "
                       f"Current balance: {to_float(current_balance):,.4f} coins (₦{to_float(coins_to_naira(current_balance)):,.2f}), "
                       f"Shortfall: {shortfall:,.4f} coins (₦{to_float(coins_to_naira(shortfall)):,.2f}). Please fund your wallet."
            )
        
        # Update interest status
        update_data = {
            "status": InterestStatus.PAID_ACCESS,
            "access_fee": to_float(base_fee_naira),
            "access_fee_vat_naira": to_float(vat_amount_naira),
            "access_fee_total_naira": to_float(total_fee_naira),
            "access_fee_total_coins": to_float(total_fee_coins),
        ...
        background_tasks.add_task(
            ...
            job=job,
            interest_id=interest_id,
            access_fee=to_float(total_fee_naira)
        )
        ...
        return {
            ...
            "access_fee_naira": to_float(base_fee_naira),
            "vat_naira": to_float(vat_amount_naira),
            "total_fee_naira": to_float(total_fee_naira),
            "access_fee_coins": to_float(total_fee_coins),
...
async def _notify_tradesperson_contact_shared(job: dict, tradesperson_id: str, interest_id: str):
    ...
    try:
        ...
        if access_fee_naira is None and access_fee_coins is not None:
            access_fee_naira = coins_to_naira(access_fee_coins)
        if access_fee_naira is None:
            access_fee_naira = 1000
        fee_breakdown = vat_inclusive_breakdown(access_fee_naira)
        vat_amount_naira = fee_breakdown["vat_naira"]
        total_fee_naira = fee_breakdown["total_naira"]
        template_data = {
            "job_id": job.get("id"),
            ...
            "access_fee": f"₦{to_float(total_fee_naira):,.2f} (VAT ₦{to_float(vat_amount_naira):,.2f})",
