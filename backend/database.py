...
class Database:
        ...
        pipeline = [
            ...
            {"$unwind": "$job"},
            {
                "$project": {
                    ...
                    "homeowner_id": {"$ifNull": ["$job.homeowner.id", "$job.homeowner_id"]},
                    "homeowner_email": {"$ifNull": ["$job.homeowner.email", ""]},
                    "homeowner_phone": {"$ifNull": ["$job.homeowner.phone", ""]},
    ...
    async def get_rejected_tradespeople_verifications(self, skip: int = 0, limit: int = 20) -> List[dict]:
        """Get rejected tradespeople verifications for admin review.
        Returns the latest rejected verification per user with rejecting admin metadata.
        """
        pipeline = [
    ...
    async def approve_tradesperson_verification(self, verification_id: str, admin_id: str, admin_notes: str = "") -> bool:
        ...
        try:
            ...
            await self._process_referral_rewards(user_id)
        except Exception:
            # Non-fatal if referral processing fails
            pass
        return True
